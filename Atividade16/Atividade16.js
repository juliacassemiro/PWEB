function abrirCurso() {
  alert("Deseja conhecer esse curso?")

  const elemento = document.getElementById("cursos");
  const opcao = elemento.options[elemento.selectedIndex];
  const idcurso = opcao.id;
  const curso = opcao.value;
  let descricao = "";
  const janela = window.open("", "curso", "width=600,height=300")
